...
-- Q2.Select customers who did not placed any order yet.
select *
from customers
...
-- Q3.Select order id together with the name of products.

select o.OrderID, p.ProductName
from order_details o join products p on o.ProductID=p.ProductID;

-- Q4.Select products that no one bought.
-- 1st try

select ProductName
from products where ProductID not in (select productid from order_details);

-- 2nd try
select p.ProductName
from shippers s left join products p on s.ShipperID=p.SupplierID
where p.SupplierID is Null
;

-- 3rd try
select ProductName
from products p  right join order_details od on p.ProductID= od.ProductID
...
-- Q5.Select customer together with the products that he bought.
...
-- Q6.Select product names together with the name of corresponding category.
select p.ProductName, c.CategoryName
from products p join categories c on p.CategoryID=c.CategoryID
;


-- Q7.Select orders together with the name of the shipping company.
select o.OrderID, o.OrderDate, s.ShipperName
from orders o join shippers s on o.ShipperID= s.ShipperID
;

-- Q8.Select customer name together with the number of orders made by the corresponding customer 
-- sort the result by number of orders in decreasing order.
select c.CustomerName, count(o.OrderID) order_cnt
from customers c join orders o on o.CustomerID=c.CustomerID
group by  c.Customername
order by order_cnt desc
;

-- Q9.Select orderID together with the total price of  that Order, order the result by total price of order in increasing order.
select o.OrderID, round(sum(o.Quantity * p.Price),2) order_price
from order_details o join products p on o.ProductID=p.ProductID
group by o.OrderID
order by order_price
;

-- Q10.Select customer who spend the most money and lives in Canada
select c.CustomerID, c.CustomerName, c.Country, round((od.Quantity * p.Price),2) order_price
from customers c join orders o on c.CustomerID=o.CustomerID
				 join order_details od on od.OrderID=o.OrderID
